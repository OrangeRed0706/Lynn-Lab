...
// This query can be #load-ed into other queries for BenchmarkDotNet support. *V=1.0*
// You can modify the code to customize benchmarking behavior.
// LINQPad will merge any subsequent updates into your modified script (after RTM).
...
/// You can change the behavior of this feature by editing this method.</summary>
			...
			if (maxDisplayMax.HasValue && (_results.Count > 1 || _results [0].GetMin() != _results [0].GetMax()))
				foreach (var r in _results)
					if (r.Operations > 0)
						...
						r.ResultsGraph = GetBar (Normalize (r.GetMean().Value));
		...
		class BenchmarkEventListener : EventListener
		{
			...
			protected override void OnEventWritten (EventWrittenEventArgs eventData)
			{
				if (eventData.EventName == "BenchmarkStart")
					CurrentCase = eventData.Payload.OfType<string>().FirstOrDefault();
			}
		}
